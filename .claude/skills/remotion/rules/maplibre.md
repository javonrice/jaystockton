	...
	useEffect(() => {
		...
		mapInstance.on('load', () => {
			...
			mapInstance.addLayer({
				...
				paint: {'line-color': '#111111', 'line-width': 7},
			});

			mapInstance.addSource('city-markers', {type: 'geojson', data: cityMarkers});
...
For zoom-out / travel / zoom-in animations, animate travel progress separately from camera altitude. Camera altitude is measured in meters.

## Lines

Use GeoJSON sources for lines. For geodesic flight routes, use Turf:
...
Use map-native GeoJSON layers for markers and labels. Make marker sizes and label font sizes large enough for the composition resolution.
